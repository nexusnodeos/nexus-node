...
export default function MineroPage() {
  ...
  return (
    <div className="min-h-screen bg-white text-[#241A14] p-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
      {/* COLUMNA IZQUIERDA: FORMULARIO */}
      <div className="bg-[#FBF6F0] border border-[#E9DFD2] rounded-xl p-6 h-fit">
        <h1 className="text-2xl font-bold text-[#B15A2A] mb-6">Portal del Minero - Registrar Mineral</h1>
      ...
      <div className="space-y-6">
        <div className="bg-[#FBF6F0] border border-[#E9DFD2] rounded-xl p-6">
          <h2 className="text-xl font-bold text-[#241A14] mb-4">Mis Lotes En Mercado</h2>
          ...
          {misLotes.length === 0 ? (
            ...
            <div className="space-y-3 max-h-60 overflow-y-auto pr-2">
              {misLotes.map((lote) => (
                <div key={lote.id} className="bg-white border border-[#E9DFD2] rounded-lg p-3 flex justify-between items-center">
                  <div>
                  ...
                  {lote.estatus === "publicado" && (
                    <button onClick={() => verOfertasDelLote(lote)} className="bg-[#F3ECE2] hover:bg-[#EADFCF] text-xs text-[#B15A2A] font-semibold py-1 px-3 rounded border border-[#E9DFD2]">
                      Ver Ofertas
                    </button>
                  )}
                </div>
        ...
        {loteSeleccionado && (
          <div className="bg-[#FBF6F0] border border-[#B15A2A]/30 rounded-xl p-6">
            <h3 className="text-lg font-bold text-[#B15A2A] mb-1">Ofertas para el Lote seleccionado</h3>
            ...
            {cargandoOfertas ? (
              ...
              <div className="space-y-3">
                {ofertasRecibidas.map((of) => (
                  <div key={of.id} className="bg-white border border-[#E9DFD2] rounded-lg p-4 flex flex-col sm:flex-row justify-between sm:items-center gap-3">
                    <div>
                    ...
                    {of.estatus === "pendiente" && (
                      <div className="flex gap-2">
                        <button onClick={() => resolverOferta(of.id, "rechazar")} className="bg-rose-50 text-rose-700 hover:bg-rose-100 border border-rose-200 text-xs font-bold py-1.5 px-3 rounded-lg transition-colors">
                          Rechazar
                        </button>
                        <button onClick={() => resolverOferta(of.id, "aceptar")} className="bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold py-1.5 px-3 rounded-lg transition-colors">
                          Aceptar
                        </button>
                      </div>
                    )}
